	...
	static{
		try {
			...
			for (int i = 0; i < airplane.length; i++) {
				airplane[i]=ImageIO.read(Main.class.getResource("/imgs/airplane"+i+".png"));
			}
			bigPlane=new BufferedImage[5];
			for (int i = 0; i < bigPlane.length; i++) {
				bigPlane[i]=ImageIO.read(Main.class.getResource("/imgs/bigplane"+i+".png"));
			}
			bee=new BufferedImage[5];
			for (int i = 0; i < bee.length; i++) {
				bee[i]=ImageIO.read(Main.class.getResource("/imgs/bee"+i+".png"));
			}
			hero=new BufferedImage[6];
			for (int i = 0; i < hero.length; i++) {
				hero[i]=ImageIO.read(Main.class.getResource("/imgs/hero"+i+".png"));
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
